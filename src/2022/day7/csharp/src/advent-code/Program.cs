var result = (await GetStacks("sample.txt")).ToArray();
...
result = (await GetStacks("measurements.txt")).ToArray();
for (var i = 0; i < result.Length; ++i)
{
...
ValueTask<IEnumerable<Stack<char>>> GetStacks(string fileName)
{
