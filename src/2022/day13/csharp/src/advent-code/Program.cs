...
var regex = new Regex($"({Regex.Escape(",")}|{Regex.Escape("[")}|{Regex.Escape("]")})");
...
async ValueTask HandleFile(string file)
{
    ...
    Console.WriteLine($"{file} has start {startIndex} and end {endIndex} with an answer of {startIndex * endIndex}");
    //Console.WriteLine(string.Join(Environment.NewLine, sortedList));
}

async ValueTask<IReadOnlyList<PacketChecker>> GetPackets(string file)
{
...
ListPacket ParseLine(string line)
{
...
record ListPacket(IReadOnlyList<IPacket> Values) : IPacket
{
...
record PacketChecker(IPacket Left, IPacket Right) : IEnumerable<IPacket>
{
