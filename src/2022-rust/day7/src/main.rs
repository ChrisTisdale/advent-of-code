...
fn main()
{
        ...
        .map(|s| s.split(" ").collect::<Vec<&str>>())
    {
        if d[0] == "$"
        {
            if d[1] == "cd"
            {
                ...
                map.insert(current_dir.to_string(),Folder
                {
                    files: Vec::new(),
                    directories: Vec::new()
                });
        ...
        if d[0] == "dir"
        {
            ...
            let value = map.get_mut(current_dir.as_str()).expect("aoc wont lie to me");
    ...
    println!("Root size: {}, need: {}, free space: {}", min_size, need_free, free_space);
...
fn find_min_size(cur: &mut usize, folder: &Folder, map: &HashMap<String, Folder>, needed: &usize, name: &String) -> Option<SizeResult>
{
