    ...
    s.chars().enumerate()
        .filter_map(|(i, c)| {
            if let Some(d) = c.to_digit(10) {
                return Some(d);
            } else if let Some(res) = matches_digit(s, i) {
                return Some(res);
            }
...
fn check_four_letter_numbers(s: &str, i: usize) -> Option<u32> {
    if s.chars().count() < i + 4 {
        return None;
    }
...
fn check_five_letter_numbers(s: &str, i: usize) -> Option<u32> {
    if s.chars().count() < i + 5 {
        return None;
    }
...
7pqrstsixteen";
    ...
    const MESSURE: &str = "six1mpffbnbnnlxthree
4eight3one92
9nine2xnhvjtjlzj48
